    ...
    public view() throws IOException {
        ...
        if(username.equals("mohammed123") && password.equals("12345!@#$%")) {
            ...
            while(!actions.equals("exit")) {
                ...
                System.out.println();
                switch (actions) {
                    case "animal" -> animal.managerActions();
                    case "employee" -> employee.managerActions();
                    case "supply" -> supply.managerActions();
                    case "income" -> income.managerActions();
                    case "expenditure" -> expenditure.managerActions();
                    case "ticket" -> ticket.managerActions();
                    case "server" -> new client();
                    case "back" -> new view();
                    case "exit" -> System.exit(0);
                    default -> System.out.println("Please choose an action from list of actions\n");
                }
            }
        }
        else {
            ...
            while(!actions.equals("exit")) {
                ...
                System.out.println();
                switch (actions) {
                    case "animal" -> animal.userActions();
                    case "ticket" -> ticket.userActions();
                    case "back" -> new view();
                    case "exit" -> System.exit(0);
                    default -> System.out.println("Please choose an action from list of actions\n");
                }
